...
// ЗАПРОС ТИПА 1: DELETE (Удаление отзыва админом по ID)
if (isset($_GET['delete_id']) && isset($_SESSION['user']) && $_SESSION['user'] === 'admin') {
    $delete_id = (int)$_GET['delete_id'];
    $stmt = $db->prepare("DELETE FROM reviews WHERE id = :id");
    $stmt->execute([':id' => $delete_id]);
    header("Location: z2.php?msg=deleted#guestbook");
    exit;
}
...
if (isset($_GET['delete_order_id']) && isset($_SESSION['user']) && $_SESSION['user'] === 'admin') {
    ...
    header("Location: z2.php?msg=order_deleted#dashboard");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Скрипт 11: Валидация формы регистрации
    ...
    // ЗАПРОС ТИПА 2: INSERT INTO (Добавление отзыва в базу)
    if (isset($_POST['gb_submit'])) {
        ...
        if ($author && $message) {
            ...
            header("Location: z2.php?msg=success#guestbook");
            exit;
        } else {
    ...
    if (isset($_POST['book_property_btn'])) {
        ...
        if ($c_name && $c_phone && $p_name) {
            ...
            header("Location: z2.php?msg=booked#properties");
            exit;
        }
    }

    // Скрипты 19-22: Интегрированный блок PHPMailer
...
// Данные карточек резиденций
$products = [
    ['id' => 1, 'name'=>'San Francisco Luxury Villa','category'=>'phones','price'=>2500000, 'img'=>'Mask group (2).png', 'rooms'=>4, 'size'=>'3,500 sq ft', 'loc'=>'San Francisco, California'],
    ['id' => 2, 'name'=>'Beverly Hills Cozy Modern','category'=>'laptops','price'=>850000, 'img'=>'Mask group (3).png', 'rooms'=>3, 'size'=>'1,500 sq ft', 'loc'=>'Beverly Hills, California'],
    ['id' => 3, 'name'=>'Palo Alto Elite Estate','category'=>'tv','price'=>3700000, 'img'=>'Mask group (4).png', 'rooms'=>6, 'size'=>'4,000 sq ft', 'loc'=>'Palo Alto, California']
];
...
<style>
    ...
    .modal-booking { background: #f0f4f8; padding: 15px; border-radius: 6px; margin-top: 10px; border-left: 4px solid #00afec; }
</style>
</head>
<body>
...
foreach ($products as $p) {
    ...
    <div class="property-card">
        ...
        <div class="property-content">
            ...
            <div class="modal-booking" style="background: #f0f4f8; padding: 12px; border-radius: 6px; border-left: 4px solid #00afec; text-align: left;">
...
<div class="container" style="max-width: 600px;">
    ...
    <div style="margin-top: 30px;">
        ...
        <?php
        // ЗАПРОС ТИПА 3: SELECT * FROM ... (Выборка всех данных для вывода)
        ...
        if (count($reviews) > 0) {
            foreach ($reviews as $row) {
                ...
                if (isset($_SESSION['user']) && $_SESSION['user'] === 'admin') {
                    echo '<a href="z2.php?delete_id=' . $row['id'] . '" onclick="return confirm(\'Удалить отзыв?\')" style="position: absolute; top: 15px; right: 15px; color: #c53929; background: #fce8e6; padding: 4px 10px; border-radius: 4px; font-size: 12px; text-decoration:none;">Удалить</a>';
                }
...
<div class="container" style="max-width: 600px;">
    ...
    <div class="string-box">
        ...
        <form method="POST" action="#mailer-section" enctype="multipart/form-data">
    ...
    <div class="string-box">
        ...
        <form method="POST" action="#mailer-section">
...
<div class="container" style="max-width: 600px;">
    ...
    <?php else: ?>
        ...
        <div class="string-box" style="background: #fdfefe; border: 1px dashed #00afec;">
            ...
            <?php
            // ЗАПРОС ТИПА 4: COUNT (Агрегатный запрос на подсчет общего числа записей)
            $count_reviews = $db->query("SELECT COUNT(*) FROM reviews")->fetchColumn();
            $count_orders = $db->query("SELECT COUNT(*) FROM orders")->fetchColumn();
            
            // ЗАПРОС ТИПА 5: WHERE (Выборка записей с фильтрацией по сегодняшней дате)
           // ЗАПРОС ТИПА 5: WHERE с функцией date() (Отрезаем время для точного сравнения дат)
        ...
        <div class="string-box" style="background: #fff; margin-top:20px;">
            ...
            if (count($orders) > 0) {
                ...
                foreach ($orders as $ord) {
                    ...
                    echo '<td style="padding:8px;"><a href="z2.php?delete_order_id=' . $ord['id'] . '" onclick="return confirm(\'Удалить заявку?\')" style="color:red; text-decoration:none;">❌ Удалить</a></td>';
                    echo '</tr>';
                }
                echo '</table>';
            } else {
...
<p>Our vision is to make more people own their dream home beautifully.</p>
</div>
...
navLinks.forEach(link => {
    link.addEventListener('click', () => {
        burgerBtn.classList.remove('remove');
